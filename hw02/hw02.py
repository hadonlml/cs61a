    ...
    def times_change(x):
        while x:
            ...
            if x % 7 == 0 or num_sevens(x) != 0:
                return 1 + times_change(x - 1)
            else:
                return 0 + times_change(x - 1)
    if n <= 7:
        return n
    if times_change(n - 1) % 2 == 1:
...
def count_change(amount):
    ...
    def partitions(n, max):
        ...
        if max == 1:
            ...
            return partitions(n - max, max) + partitions(n, max // 2)
    return partitions(amount, compute_pow(amount))
...
def make_anonymous_factorial():
    ...
    True
    """
    return (lambda f: lambda n: f(f, n))(lambda f1, n: 1 if n == 1 else mul(n, f1(f1, n - 1)))
